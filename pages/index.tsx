...
export default function Home() {
  ...
  const toUtcTime = (ts: any) => {
    return new Date(ts * 1000).toISOString().substring(11, 16);
  };
  ...
  const getData = async () => {
    try {
      ...
      setData(convertTimestampsToUnix(data));
    } catch (err) {
    ...
    <div className={`h-dvh w-dvw`}>
      <main className="flex h-full w-full flex-col gap-2 overflow-y-scroll p-2">
        ...
        <div className={`font-key flex flex-col gap-1 rounded-xl border p-4`}>
          ...
          <div className={`grid h-16 w-full grid-cols-2 gap-2 overflow-x-scroll rounded-xl border px-2 break-keep`}>
            <div className={`flex h-full w-full items-center justify-start gap-4 text-xs md:text-base`}>
              ...
              <span className={`${getChangeRate().bidsRate >= 0 ? "text-green-600" : "text-red-600"}`}>{`${getChangeRate().bidsRate >= 0 ? "+" : "-"}${getChangeRate().bidsRate.toFixed(2)}`}</span>
            </div>
            ...
            <div className={`flex h-full w-full items-center justify-start gap-4 text-xs md:text-base`}>
              ...
              <span className={`${getChangeRate().pointsRate >= 0 ? "text-green-600" : "text-red-600"}`}>{`${getChangeRate().pointsRate >= 0 ? "+" : "-"}${getChangeRate().pointsRate.toFixed(2)}`}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`font-key w-full rounded-xl border p-2`}>
          ...
          <div className={`flex h-fit w-full flex-col overflow-y-scroll rounded-xl border px-2 text-xs md:text-base`}>
